...
public class Scanner{
    private string source { get; private set; }
    ...
    public Scanner(string source){
        ...
        reserved["Stringtype"] = new Token(TokenType.STRsINGTYPE, "Stringtype", 0,0 );
...
private void scanToken(){
    ...
    switch (c){
        ...
        case ')': addToken(TokenType.RIGTH_PAREN); break;
        case '{': addToken(TokenType.LEFT_BRACE); break;
        case '}': addToken(TokenType.RIGTH_BRACE); break;
        ...
        case '|': if(Macth('|')) addToken(TokenType.OR);
        case '&': if(Macth('&')) addToken(TokenType.OR);
        case '=':
...
private void number(){
    while(isDigiti(peek())) advance();
    if(peek() == '.'&& isDigiti(peekNext())){
        advance();
        while(isDigit(peek())) advance();
    }
    addToken(TokenType.TokenType.NUMBER, Double.parseDouble(source.substring(start,current)));
}
